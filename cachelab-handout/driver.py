#!/usr//bin/python
#
#i  driver.py - The driver tests the correctness of the student's cache
...
#    trans_cscore = int(result32[0]) * int(result64[0]) * int(result61[0]);
    trans_cscore = int(result32[0]) * int(result61[0]);
...
#    total_score = csim_cscore[0] + trans32_score + trans64_score + trans61_score
    total_score = csim_cscore[0] + trans32_score + trans61_score
...
#        autoresult="%.1f:%d:%d:%d" % (total_score, miss32, miss64, miss61)
        autoresult="%.1f:%d:%d" % (total_score, miss32, miss61)
